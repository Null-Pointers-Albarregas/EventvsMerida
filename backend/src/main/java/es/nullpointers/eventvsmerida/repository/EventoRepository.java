...
public interface EventoRepository extends JpaRepository<Evento, Long> {
    ...
    Optional<Evento> findByTituloAndFechaInicioAndFechaFin(String titulo, LocalDateTime fechaInicio,
            LocalDateTime fechaFin);

    /*
     * @Query("SELECT e FROM Evento e JOIN e.categoria c WHERE " +
     * "LOWER(e.titulo) LIKE CONCAT('%', LOWER(:q), '%') OR " +
     * //"LOWER(e.descripcion) LIKE CONCAT('%', LOWER(:q), '%') OR " +
     * "LOWER(e.localizacion) LIKE CONCAT('%', LOWER(:q), '%') OR " +
     * "LOWER(c.nombre) LIKE CONCAT('%', LOWER(:q), '%')")
